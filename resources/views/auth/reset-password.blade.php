...
<div class="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-100 via-blue-300 to-blue-500">
    <div class="bg-white bg-opacity-90 rounded-xl shadow-lg p-10 max-w-md w-full text-center">
        ...
        <h1 class="text-3xl font-bold text-blue-700 mb-2">Restablecer contraseña</h1>
        <p class="text-gray-600 mb-6">Ingresá tu nueva contraseña para acceder a Swiiss Médicos</p>
        <form method="POST" action="{{ route('password.store') }}" class="text-left">
            @csrf
            <input type="hidden" name="token" value="{{ $request->route('token') }}">
            <div class="mb-4">
                <label for="email" class="block text-gray-700 font-semibold mb-1">Email</label>
                <input id="email" type="email" name="email" value="{{ old('email', $request->email) }}" required autofocus autocomplete="username" class="w-full px-4 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-400">
                @error('email')
            ...
            <button type="submit" class="w-full py-2 px-4 bg-blue-600 text-white font-semibold rounded hover:bg-blue-700 transition">Restablecer contraseña</button>
        </form>
    </div>
